      ...
      {(label || showPercent) && (
        <div className="flex justify-between items-center mb-1">
          ...
          {showPercent && !isIndeterminate && (
            <span className="text-xs text-[var(--color-text-muted)]">{Math.round(value!)}%</span>
          )}
        </div>
      )}
      <div
        ...
        aria-label={label}
        className="h-1.5 w-full rounded-full bg-[var(--color-surface-raised)] overflow-hidden"
      >
        {isIndeterminate ? (
          <div className="h-full w-1/3 rounded-full bg-[var(--color-primary)] animate-[indeterminate_1.5s_ease-in-out_infinite]" />
        ) : (
          <div
