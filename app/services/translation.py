...
def translate_segments(segments: list, source_lang: str, target_lang: str,
    ...
    total = len(segments)
    result = []
    log_task_event(task_id, "translate_start",
                   source=source_lang, target=target_lang, segments=total)
    ...
    for i, seg in enumerate(segments):
        ...
        if task_id and (i + 1) % TRANSLATION_BATCH_SIZE == 0 or i == total - 1:
            ...
            emit_event(task_id, "translate_progress", {
                "percent": pct,
                "translated": i + 1,
                "total": total,
    ...
    log_task_event(task_id, "translate_done",
                   source=source_lang, target=target_lang, segments=total)
    return result
