...
def cleanup_old_files(directory: Path, max_age_seconds: int = DEFAULT_RETENTION_SECONDS,
...
async def periodic_cleanup(retention_seconds: int = DEFAULT_RETENTION_SECONDS):
    ...
    logger.info(f"CLEANUP Background cleanup started (retention={retention_seconds/3600:.1f}h, "
    ...
    while True:
        try:
            ...
            upload_result = await asyncio.to_thread(
                cleanup_old_files, UPLOAD_DIR, retention_seconds
            )
            output_result = await asyncio.to_thread(
                cleanup_old_files, OUTPUT_DIR, retention_seconds
            )
            ...
            if total_removed > 0:
                ...
                log_task_event("system", "cleanup",
                               uploads=upload_result, outputs=output_result,
                               total_removed=total_removed, total_freed_mb=round(total_freed / 1024 / 1024, 1))
